...
- **Multiple concurrent sessions.** Currently only one session is active globally. Per-connection session selection (e.g. via a request header or path prefix) would allow multiple clients to use different providers simultaneously.
